...
class DeepNeuralNetworkDropconnect(BasicConfig):
    ...
    def data_manager(self):
        '''
            Randomly separate the data to train_data or test_data according to the train_ratio
        '''
        train_input_data = []
        train_output_data = []
        test_input_data = []
    ...
    def model(self):
        ...
        tf.add_to_collection("istrain", self.istrain)
        ...
        self.learning_rate = tf.Variable(5e-2, trainable = False)
    ...
    def run(self):
        ...
        print("Accuracy : {}".format(self.sess.run(self.accuracy, feed_dict = {self.x : self.test_data['input'],
                                                                               self.y : self.test_data['output'],
                                                                               self.istrain : False})))
        for i in range(self.epoch):
            ...
            train_cost = self.sess.run(self.error, feed_dict = {self.x : self.train_data['input'],
                                                                self.y : self.train_data['output'],
                                                                self.istrain : False}) 
            test_cost = self.sess.run(self.error, feed_dict = {self.x : self.test_data['input'],
                                                               self.y : self.test_data['output'],
                                                               self.istrain : False})
        ...
        print("Accuracy : {}".format(self.sess.run(self.accuracy, feed_dict = {self.x : self.test_data['input'],
                                                                               self.y : self.test_data['output'],
                                                                               self.istrain : False})))
        print("Dropconnect Accuracy : {}".format(self.sess.run(self.accuracy, feed_dict = {self.x : self.test_data['input']
            , self.y : self.test_data['output']
            , self.istrain : True})))
...
if __name__ == '__main__':
    ...
    save_dir = '../../models_save/DNN_dropconnect/f{}b{}log{}comp{}'.format(forward_diff, backward_diff, islog, comp_ratio) # Store file here
    ...
    DNND = DeepNeuralNetworkDropconnect(
